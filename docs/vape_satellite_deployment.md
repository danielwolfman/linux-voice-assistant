...
When `codex.dispatch_mode` is `app_server`, backend jobs are sent to an existing Codex app-server over `codex.app_server_url`. The app-server client identity defaults to the same client name/version used by ChatGPT Android remote-control sessions (`codex_chatgpt_android_remote` / `dev`), and the backend omits `threadSource` and `serviceName` by default so locally-dispatched jobs look like remote app-originated threads in Codex's local thread database. Override this with `codex.app_server_client_name`, `codex.app_server_client_version`, `codex.app_server_thread_source`, or `codex.app_server_service_name` only if a different app-server client should be identified explicitly.
...
test -r /home/YOUR_USER/.codex && test -w /home/YOUR_USER/.codex && test -x /home/YOUR_USER/.codex
```
